...
$(document).ready(function() {
	...
	if (SC.isConnected()) {
		SC.get('/me', function(me) {
			$('#nowplaying').after('<input type="button" class="statusbar ctrl login" id="userLogoff" value="' + me.username + '">');
		});
	} else {
	...
	list.prototype.push = function(track) {
		this.trackCount += 1;
		this.tracks.push(track);
	};

	list.prototype.insert = function(track, i) {
	...
	function pushToList(list, track, index) {
		...
		if (arguments.length < 3 || index >= list.length) {
			...
			for (i = list.length - 1; i > index; i -= 1) {
	...
	function playTrack(list, track) {
		...
		curTrackID = currentPlaylist.tracks[currentPlaylistTrack].id;

		// if (currentPlaylist[currentPlaylistTrack].id) {
		if (curTrackID) {
			SC.stream("/tracks/" + curTrackID, {
					onload: function() {
						console.log("onload");
					},
					onfinish: function() {
